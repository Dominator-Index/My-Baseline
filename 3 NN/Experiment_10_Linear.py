...
# 创建图片保存文件夹
IMAGE_SAVE_DIR = "/home/ouyangzl/BaseLine/Exp10/images"
...
# 初始化 wandb
wandb.init(project=config["swanlab_project_name"], name=f"Linear{num_layer }+{method}+lr{lr}+var{var:.6f}_rank{rank}_top{top_k}_raw", api_key="zrVzavwSxtY7Gs0GWo9xV")
...
def plot_training_loss(loss_history, step_history, method, lr, var, seed, rank, num_layer):
    """绘制训练损失图"""
    ...
    # 保存图片
    plot_filename = os.path.join(IMAGE_SAVE_DIR, f"linear{num_layer}_training_loss_{method}_lr{lr}_var{var:.6f}_rank{rank}_seed{seed}_raw.png")
...
def plot_top_k_eigenvalues(eigenvalue_history, step_history, top_k, method, lr, var, seed, rank, num_layer):
    ...
    # 保存图片
    plot_filename = os.path.join(IMAGE_SAVE_DIR, f"linear{num_layer }_top{top_k}_eigenvalues_raw_{method}_lr{lr}_var{var:.6f}_rank{rank}_seed{seed}.png")
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
...
def plot_dominant_dims(dominant_dim_history, step_history, top_k, method, lr, var, seed, rank, num_layer ):
    """绘制主导维度演化图"""
    ...
    plot_filename = os.path.join(IMAGE_SAVE_DIR, f"linear{num_layer }_top{top_k}_dominant_space_dimension_{method}_lr{lr}_var{var:.6f}_rank{rank}_seed{seed}.png")
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
...
for seed_idx, seed in enumerate(seeds):
    ...
    dominant_dim_history = []  # 主导维度历史 (修复拼写)
    ...
    for step in progress_bar:
        ...
        if step in selected_steps:
            ...
            try:
                ...
                dominant_dim, gaps = search_top_k_dominant_space(eigenvalues, method=config["method"])

                # 记录 dominant dim 和 gaps
                dominant_dim_history.append(dominant_dim)
                gaps_history.append(gaps[:5])



                print(f"📊 Step {step}: 计算 Hessian 特征值成功, Dominant Dimension: {dominant_dim}, Gaps: {gaps[:5]}...")
                wandb.log({
    ...
    try:
        ...
        # 3. 主导维度演化图 (修复拼写错误)
        if len(dominant_dim_history) > 0:  # 这里保持原变量名
            plot_dominant_dims(dominant_dim_history, eigenvalue_step_history, top_k, method, lr, var, seed, rank)
