    ...
    for step in range(steps + 1):
        ...
        if step in selected_steps:
            ...
            for layer_name, eigenvals in layer_eigenvalues.items():
                ...
                layer_log_dict[f"max_eigenval_{layer_name}_{method}_{lr}_{var}_{seed}"] = max_eigenval
            
            # 继续计算 Hessian 特征值（保持原有功能）
            ...
            layer_log_dict[f"max_hessian_auto_{method}_{lr}_{var}_{seed}"] = max_ev
            
            # 使用相同的 step，但不让 wandb 自动递增
            wandb.log(layer_log_dict, step=step, commit=False)
            wandb.log({}, commit=True)  # 提交这一步的所有数据
