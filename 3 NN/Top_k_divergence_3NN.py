...
from model_3NN import LinearNetwork
...
# 初始化 wandb
wandb.init(project=config["wandb_project_name"], name=f"3NN+{method}+lr{lr}+var{var:.6f}_rank{rank}_top{top_k}_raw", api_key="zrVzavwSxtY7Gs0GWo9xV")
...
def plot_training_loss(loss_history, step_history, method, lr, var, seed, rank):
    ...
    # 保存图片
    plot_filename = os.path.join(IMAGE_SAVE_DIR, f"manual_3NN_training_loss_{method}_lr{lr}_var{var:.6f}_rank{rank}_seed{seed}_raw.png")
...
def plot_top_k_eigenvalues(eigenvalue_history, step_history, top_k, method, lr, var, seed, rank):
    ...
    # 保存图片
    plot_filename = os.path.join(IMAGE_SAVE_DIR, f"manual_3NN_top{top_k}_eigenvalues_raw_{method}_lr{lr}_var{var:.6f}_rank{rank}_seed{seed}.png")
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
    ...
    for step in progress_bar:
        ...
        if step in selected_steps:
            ...
            try:
                ...
                output = model(data)
                loss = loss_function(output, label)
                eigenvalues = compute_hessian_eigen(
                    loss=loss,
