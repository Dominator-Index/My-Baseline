...
def compute_hessian_eigen_pyhessian(model, criterion, data_loader, top_k=5, device=device):
    ...
    """
    hessian_computer = hessian(model=model, criterion=criterion, dataloader=data_loader)
    ...
    return hessian_eigen[0], hessian_eigen[1]
...
def compute_hessian_eigenvalues_pyhessian(model, criterion, data_loader, top_k=200, device='cuda'):
    """
    ...
    # 只返回特征值
    hessian_eigen= hessian_computer.eigenvalues(maxIter=1000, tol=1e-6, top_n=top_k, )
    eigenvalues = np.array(hessian_eigen[0])
    return eigenvalues
